          ...
          {links.map((link) => {
            ...
            return (
              <button key={link.id} onClick={() => { setCurrentView(link.id); setIsOpen(false); }}
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl mb-1 text-sm font-semibold transition-all duration-200 ${active
          ...
          <div key={game.id} className="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm hover:shadow-md transition-all group">
            <div className="p-5">
              <div className="flex items-start justify-between mb-3">
                ...
                <span className={`px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider ${game.status === "completed"
      ...
      <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm">
        {sorted.map((team, idx) => (
          <div key={team.id} className={`flex items-center gap-4 p-4 md:p-5 ${idx > 0 ? "border-t border-slate-100" : ""} hover:bg-slate-50/50 transition-colors`}>
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm font-black ${idx === 0 ? "bg-amber-100 text-amber-700" : idx === 1 ? "bg-slate-200 text-slate-600" : idx === 2 ? "bg-orange-100 text-orange-600" : "bg-slate-100 text-slate-400"
              }`}>
              {idx + 1}
            </div>
